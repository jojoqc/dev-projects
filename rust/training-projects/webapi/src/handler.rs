...
impl PostPostHandler{
    fn new(database: Arc<Mute<Database>>)->PostFeedHandler{
        PostFeedHandler{ database}
    }
}
...
impl Handler for PostHandler{
    fn handle(&self, req: &mut Request)-> IronResult<Response>{
        ...
        if let Some(post) = self.find_post(&id){
            ...
            Ok(Respons::with((status::NotFound)))
        }
    }
}

pub struct JsonAfterMiddleware;
impl JsonAfterMiddleware for JsonAfterMiddleware{
    fn after(&self, _: &mut Resquest, mut res: Response) -> IronResult<Response>{
